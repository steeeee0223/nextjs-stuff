...
const Toolbar = ({ board, preview }: ToolbarProps) => {
  /** Input */
  ...
  const deleteFile = async (onComplete?: () => Promise<void>) => {
    try {
      ...
      console.log(`[edgestore] file with url not found: ${board.coverImage}`);
    }
    await onComplete?.();
  };
  /** Action - update */
  const update = async (data: Partial<Document>) =>
  ...
  const onIconSelect = (icon: string) => update({ id: board.id, icon });
  const onRemoveIcon = () => update({ id: board.id, icon: null });
  const onUploadCover = async (file: File) => {
    const res = await edgestore.publicFiles.upload({
      file,
      options: { replaceTargetUrl: board.coverImage ?? undefined },
    });
    console.log(`uploaded to edgestore: ${res.url}`);
    update({ id: board.id, coverImage: res.url }).catch((e) => console.log(e));
  };
  const onUnsplashCover = async (url: string) =>
    await deleteFile(() => update({ id: board.id, coverImage: url }));
  const onRemoveCover = async () =>
    await deleteFile(() => update({ id: board.id, coverImage: null }));
  /** Props */
    ...
    <>
      ...
      <div className="mx-auto md:max-w-3xl lg:max-w-4xl">
        <div className="group relative pl-[54px]">
          ...
          {!!board.icon && preview && (
            <p className="pt-6 text-6xl">{board.icon}</p>
          )}
          <div
            className={cn(
              theme.flex.gap1,
